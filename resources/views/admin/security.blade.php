...
@section('title', '登录安全监控 - 运营管理后台')
@section('mobile-title', '安全监控')
...
<!-- 页面标题 -->
<div class="mb-6">
    <h2 class="text-xl sm:text-2xl font-bold text-gray-900">登录安全监控</h2>
    <p class="text-gray-600 text-sm sm:text-base">监控管理员登录安全状况和异常尝试</p>
</div>
...
<div class="bg-white shadow rounded-lg mb-8">
    <div class="px-4 py-5 sm:p-6">
        ...
        <div class="mb-6 overflow-x-auto">
            <div class="min-w-0" style="min-height: 250px;">
    ...
    <div class="bg-white shadow rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            ...
            <div class="max-h-64 overflow-y-auto">
                ...
                <!-- 移动端列表 -->
                <div class="sm:hidden" id="abnormalIpsMobileList">
                    <!-- 动态加载 -->
                </div>
            </div>
        </div>
    ...
    <div class="bg-white shadow rounded-lg">
        <div class="px-4 py-5 sm:p-6">
            ...
            <div class="max-h-64 overflow-y-auto">
                ...
                <!-- 移动端列表 -->
                <div class="sm:hidden" id="abnormalEmailsMobileList">
                    <!-- 动态加载 -->
                </div>
            </div>
        </div>
    ...
    <div class="px-4 py-5 sm:p-6">
        ...
        <form id="unlockForm" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="flex items-end">
                <button type="submit" class="w-full bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 text-sm">
                    解锁账户
                </button>
            </div>
        </form>
...
async function loadAbnormalIps() {
    try {
        ...
        // 更新移动端列表
        const mobileList = document.getElementById('abnormalIpsMobileList');
        if (mobileList) {
            mobileList.innerHTML = '';
            
            if (data.abnormal_ips.length === 0) {
                mobileList.innerHTML = '<div class="p-4 text-center text-gray-500 text-sm">暂无异常IP</div>';
            } else {
                data.abnormal_ips.forEach(item => {
                    ...
                    card.innerHTML = `
                        <div class="flex justify-between items-center">
                            <div class="flex-1 min-w-0">
                                <div class="text-sm font-mono text-gray-900">${item.ip}</div>
...
async function loadAbnormalEmails() {
    try {
        ...
        // 更新移动端列表
        const mobileList = document.getElementById('abnormalEmailsMobileList');
        if (mobileList) {
            mobileList.innerHTML = '';
            
            if (data.abnormal_emails.length === 0) {
                mobileList.innerHTML = '<div class="p-4 text-center text-gray-500 text-sm">暂无异常邮箱</div>';
            } else {
                data.abnormal_emails.forEach(item => {
                    const card = document.createElement('div');
                    card.className = 'border-b border-gray-200 p-3';
                    card.innerHTML = `
                        <div class="flex justify-between items-start">
                            <div class="flex-1 min-w-0">
                                <div class="text-sm text-gray-900 truncate">${item.email}</div>
                                <div class="text-xs text-gray-500 mt-1">${item.ips.length} 个IP地址</div>
                            </div>
                            <div class="flex-shrink-0 ml-2">
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                    ${item.attempts} 次
